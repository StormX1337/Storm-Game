...
export default async function serverRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.patch('/:id', { schema: { tags: ['Servers'] } }, async (request) => {
    ...
    if (
      !wantsAdminChange &&
      !access.permissions.has(Permission.SERVERS_UPDATE) &&
      !access.isOwner
    ) {
  ...
  app.delete(
    '/:id',
    { schema: { tags: ['Servers'], summary: 'Permanently delete a server' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      ...
      if (!access.isAdmin && !access.isOwner)
        throw forbidden('Only the owner or an admin can delete a server');
      ...
      try {
        ...
        app.log.warn(
          ...
          'force-deleting server despite node error',
        );
      }
  ...
  app.post(
    '/:id/power',
    { schema: { tags: ['Servers'], summary: 'Start, stop, restart or kill' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
  ...
  app.post(
    '/:id/command',
    { schema: { tags: ['Servers'], summary: 'Send a console command' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      ...
      await app.agents.request(
        access.server.node,
        `/api/v1/servers/${access.server.uuid}/command`,
        {
          method: 'POST',
          body: { command },
        },
  ...
  app.post('/:id/reinstall', { schema: { tags: ['Servers'] } }, async (request) => {
    ...
    if (
      access.server.status === ServerStatus.INSTALLING ||
      access.server.status === ServerStatus.REINSTALLING
    ) {
  ...
  app.get(
    '/:id/stats',
    { schema: { tags: ['Servers'], summary: 'Live resource usage' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      ...
      const stats = await app.agents
        .request<ServerLiveStats>(
          access.server.node,
          `/api/v1/servers/${access.server.uuid}/stats`,
          {
            timeoutMs: 5000,
          },
        )
  ...
  app.get(
    '/:id/stats/history',
    { schema: { tags: ['Servers'], summary: 'Historical resource usage' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      const q = query(
        request,
        z.object({ hours: z.coerce.number().int().min(1).max(168).default(6) }),
      );
      const access = await app.serverAccess.require(user, id, Permission.SERVERS_VIEW);
  ...
  app.patch(
    '/:id/startup',
    { schema: { tags: ['Servers'], summary: 'Change image or startup command' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
  ...
  app.put(
    '/:id/variables',
    { schema: { tags: ['Servers'], summary: 'Update environment variables' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
  ...
  app.get('/:id/allocations', { schema: { tags: ['Servers'] } }, async (request) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
    const access = await app.serverAccess.require(
      user,
      id,
  ...
  app.post(
    '/:id/allocations',
    { schema: { tags: ['Servers'], summary: 'Claim an additional port' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      ...
      const claimed = await app.prisma.$transaction(async (tx) => {
        ...
        if (!target) {
          throw new AppError(
            ...
            'No free port is available on this node',
          );
        }
        ...
        if (result.count === 0) {
          throw new AppError(
            409,
            ErrorCode.NO_ALLOCATION_AVAILABLE,
            'That port was just taken, try another',
          );
        }
  ...
  app.post(
    '/:id/allocations/:allocationId/primary',
    { schema: { tags: ['Servers'] } },
    async (request) => {
      const user = request.currentUser();
      const { id, allocationId } = params(
      ...
      await app.prisma.$transaction([
        ...
        app.prisma.serverAllocation.update({
          where: { id: allocationId },
          data: { isPrimary: true },
        }),
      ]);
  ...
  app.get(
    '/:id/sftp',
    { schema: { tags: ['Servers'], summary: 'SFTP connection details' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
  ...
  app.post(
    '/:id/sftp/reset',
    { schema: { tags: ['Servers'], summary: 'Rotate the SFTP password' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      const access = await app.serverAccess.require(user, id, Permission.SERVERS_SFTP);
      if (!access.isOwner && !access.isAdmin)
        throw forbidden('Only the owner can rotate SFTP credentials');
      ...
      await app.audit.activity(request, {
        serverId: access.server.id,
        event: 'server:sftp.password_reset',
      });
  ...
  app.post(
    '/:id/subusers',
    { schema: { tags: ['Servers'], summary: 'Share a server with another user' } },
    async (request) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      const input = body(
      ...
      if (!target)
        throw notFound('No user with that email address exists', ErrorCode.USER_NOT_FOUND);
      if (target.id === access.server.ownerId)
