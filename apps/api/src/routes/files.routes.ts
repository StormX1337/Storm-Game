...
export default async function fileRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/:id/files/list', { schema: { tags: ['Files'], summary: 'List a directory' } }, async (request) => {
  ...
  app.get('/:id/files/contents', { schema: { tags: ['Files'], summary: 'Read a text file' } }, async (request, reply) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
  ...
  app.get('/:id/files/download', { schema: { tags: ['Files'], summary: 'Download a file' } }, async (request, reply) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
    const q = query(request, z.object({ path: z.string().min(1).max(4096) }));
    const access = await app.serverAccess.require(user, id, Permission.SERVERS_FILES);
    ...
    void reply
      ...
      .header('content-disposition', `attachment; filename="${sanitizeFilename(path.split('/').pop() ?? 'download')}"`);
  ...
  app.get('/:id/files/search', { schema: { tags: ['Files'], summary: 'Search for files by name' } }, async (request) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
  ...
  app.post('/:id/files/write', { schema: { tags: ['Files'], summary: 'Create or overwrite a text file' } }, async (request) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
    ...
    await app.agents.request(access.server.node, `/api/v1/servers/${access.server.uuid}/files/write`, {
      method: 'POST',
  ...
  app.post('/:id/files/upload', { schema: { tags: ['Files'], summary: 'Upload files' } }, async (request) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
    const access = await app.serverAccess.require(user, id, Permission.SERVERS_FILES_WRITE);
    ServerAccessService.assertNotSuspended(access);
    ...
    for await (const part of request.parts()) {
      ...
      await app.agents.request(access.server.node, `/api/v1/servers/${access.server.uuid}/files/upload`, {
        method: 'POST',
        query: { path: target },
  ...
  app.post('/:id/files/rename', { schema: { tags: ['Files'] } }, async (request) => {
    ...
    await app.agents.request(access.server.node, `/api/v1/servers/${access.server.uuid}/files/rename`, {
      method: 'POST',
  ...
  app.post('/:id/files/copy', { schema: { tags: ['Files'] } }, async (request) => {
    ...
    await app.agents.request(access.server.node, `/api/v1/servers/${access.server.uuid}/files/copy`, {
      method: 'POST',
      body: {
  ...
  app.post('/:id/files/delete', { schema: { tags: ['Files'] } }, async (request) => {
    ...
    await app.agents.request(access.server.node, `/api/v1/servers/${access.server.uuid}/files/delete`, {
      method: 'POST',
  ...
  app.post('/:id/files/compress', { schema: { tags: ['Files'], summary: 'Create a zip archive' } }, async (request) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
  ...
  app.post('/:id/files/decompress', { schema: { tags: ['Files'], summary: 'Extract an archive' } }, async (request) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
  ...
  app.post('/:id/files/chmod', { schema: { tags: ['Files'], summary: 'Change file permissions' } }, async (request) => {
    const user = request.currentUser();
    const { id } = params(request, idParam);
    ...
    await app.agents.request(access.server.node, `/api/v1/servers/${access.server.uuid}/files/chmod`, {
      method: 'POST',
