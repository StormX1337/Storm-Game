...
export default async function serverDatabaseRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post(
    '/:id/databases',
    { schema: { tags: ['Databases'], summary: 'Create a database' } },
    async (request, reply) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      ...
      const existing = await app.prisma.serverDatabase.count({
        where: { serverId: access.server.id },
      });
      ...
      if (host.maxDatabases > 0) {
        ...
        if (used >= host.maxDatabases)
          throw badRequest('That database host has reached its capacity');
      }
  ...
  app.post(
    '/:id/databases/:databaseId/rotate',
    { schema: { tags: ['Databases'], summary: 'Reset the password' } },
    async (request) => {
      const user = request.currentUser();
      ...
      const password = generatePassword(28);
      await app.databases.resetPassword(
        record.host,
        record.username,
        password,
        record.remoteAccess,
  ...
  app.get(
    '/:id/databases/:databaseId/credentials',
    { schema: { tags: ['Databases'] } },
    async (request) => {
      const user = request.currentUser();
      const { id, databaseId } = params(request, dbParam);
      const access = await app.serverAccess.require(user, id, Permission.SERVERS_DATABASES);
