...
export default async function backupRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    '/:id/backups',
    { schema: { tags: ['Backups'], summary: 'List backups for a server' } },
    async (request) => {
  ...
  app.post(
    '/:id/backups',
    { schema: { tags: ['Backups'], summary: 'Create a backup' } },
    async (request, reply) => {
      const user = request.currentUser();
      const { id } = params(request, idParam);
      ...
      if (!storage)
        throw new AppError(503, ErrorCode.STORAGE_ERROR, 'No backup storage is configured');
  ...
  app.patch(
    '/:id/backups/:backupId',
    { schema: { tags: ['Backups'], summary: 'Rename or lock a backup' } },
    async (request) => {
      const user = request.currentUser();
      const { id, backupId } = params(request, backupParam);
      const input = body(
  ...
  app.get(
    ...
    { schema: { tags: ['Backups'], summary: 'Download a backup archive' } },
    async (request, reply) => {
      const user = request.currentUser();
      const { id, backupId } = params(request, backupParam);
      const access = await app.serverAccess.require(user, id, Permission.SERVERS_BACKUPS);
  ...
  app.post(
    ...
    { schema: { tags: ['Backups'], summary: 'Restore a backup' } },
    async (request) => {
      const user = request.currentUser();
      const { id, backupId } = params(request, backupParam);
      ...
      if (!backup) throw notFound('Backup was not found', ErrorCode.BACKUP_NOT_FOUND);
      if (backup.status !== BackupStatus.COMPLETED)
        throw badRequest('That backup is not restorable');
  ...
  app.delete('/:id/backups/:backupId', { schema: { tags: ['Backups'] } }, async (request) => {
    ...
    if (backup.status === BackupStatus.RUNNING)
      throw conflict('That backup is still being created');

    await app.prisma.backup.update({
      where: { id: backupId },
      data: { status: BackupStatus.DELETING },
    });
    ...
    try {
      ...
      app.log.warn(
        { err: error, backupId },
        'failed to remove backup archive; deleting record anyway',
      );
    }
