...
export default async function dashboardRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    '/overview',
    { schema: { tags: ['Dashboard'], summary: 'Customer dashboard metrics' } },
    async (request) => {
      ...
      const overview: DashboardOverview = {
        servers: {
          ...
          offline: servers.filter(
            (server) =>
              server.status === ServerStatus.OFFLINE || server.status === ServerStatus.CRASHED,
          ).length,
        ...
        nodes.map(async (node) => {
          ...
          return {
            ...
            availableMemory: Math.max(
              0,
              Math.floor(node.memoryTotal * (1 + node.memoryOvercommit / 100)) -
                summary.allocatedMemory,
            ),
  ...
  app.get(
    '/nodes/:id/stats',
    { schema: { tags: ['Nodes'], summary: 'Live node utilisation' } },
    async (request) => {
      const user = request.currentUser();
  ...
  app.get(
    '/templates',
    { schema: { tags: ['Templates'], summary: 'Game templates available to customers' } },
    async (request) => {
      const q = query(
        request,
        z.object({
