...
export default async function adminTemplateRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/', { schema: { tags: ['Admin: Templates'] } }, async (request) => {
    const q = query(request, paginationQuerySchema.extend({ category: z.string().max(64).optional() }));
  ...
  app.post('/', { schema: { tags: ['Admin: Templates'], summary: 'Create a template' } }, async (request, reply) => {
  ...
  app.post('/:id/clone', { schema: { tags: ['Admin: Templates'], summary: 'Duplicate a template' } }, async (request, reply) => {
    const { id } = params(request, idParam);
    const input = body(
      request,
      z.object({
  ...
  app.get('/:id/export', { schema: { tags: ['Admin: Templates'], summary: 'Export as JSON' } }, async (request) => {
    const { id } = params(request, idParam);
    const template = await app.prisma.gameTemplate.findUnique({
      where: { id },
  ...
  app.post('/import', { schema: { tags: ['Admin: Templates'], summary: 'Import a template JSON' } }, async (request, reply) => {
    const input = body(request, createTemplateSchema);
